    ...
    bool parseAllFrameGroups(IOBuffer& buffer, std::vector<FrameGroup>& frameGroups)
    {
        ...
        while (bytesAvailable)
        {
            ...
            for (uint32_t atlasIndex = 0; atlasIndex < context.getVps().getAtlasCountMinus1() + 1; atlasIndex++)
            {
                ...
                int result = decode(&context, atlasIndex);
                ...
                size_t numFrames = context.getFrames().size();
                ...
                {
                    pcc::PCCFrameContext& frameContext = context.getFrames().at(frameIndex);
                    
                    FrameData& frame = frameGroup.frames[frameIndex];
                    frame.afOrderCnt = frameIndex;
                    frame.index = frameIndex;
                    frame.width = frameContext.getWidth();
                    frame.height = frameContext.getHeight();
