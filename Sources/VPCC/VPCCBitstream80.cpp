...
namespace VPCC
{
    ...
    namespace BitstreamReader
    {
        ...
        uint32_t readBits(Bitstream& bs, size_t count)
        {
            ...
            LOG_V("PCCBitstream::read = bits: %d", count);
		...
		int32_t readBitsS(Bitstream& bs, size_t count)
        {
            ...
            uint32_t code = readBits(bs, count);
            int32_t value;

            uint32_t midPoint = (1 << (count - 1));
            if (code < midPoint)
            {
                value = (int32_t) code;
            }
		...
		std::string readString(Bitstream& bs)
        {
            ...
            std::string str;
            char element = readBits(bs, 8);
			
            while (element != 0x00)
            {
                str.push_back(element);
                element = readBits(bs, 8);
            }
        ...
        void skipBytes(Bitstream& bs, size_t count)
        {
            ...
            LOG_E("PCCBitstream::skipBytes = bytes: %d", count);
