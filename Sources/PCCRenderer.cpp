...
void PCCRenderer::create(bool manualVideoTextureUpload)
{
    // Shader
    char kVertexShaderFilename[] = "Assets/Shaders/pcc.vert";
    char kFragmentShaderFilename[] = "Assets/Shaders/pcc.frag";
...
void PCCRenderer::draw(PresentationFrame& presentationFrame, glm::mat4 model, glm::mat4 view, glm::mat4 projection, glm::mat4 mvp, glm::vec3 offset, float scale)
{
    pushDebugMarker("PCCRenderer Init");
...
#if EXPORT_PATCHES
    {
        ...
        {
            PCC::Patch& patch = presentationFrame.patches[patchIndex];
    ...
    for (size_t patchIndex = 0; patchIndex < numPatches; patchIndex++)
    {
        PCC::Patch& p = presentationFrame.patches[patchIndex];
        ...
        for (uint32_t by = 0; by < p.sizeV0; by++)
        {
            ...
            {
                size_t blockToPatchIndex = PCC::patchBlockToCanvasBlock(p, bx, by, blockToPatchWidth, blockToPatchHeight);
    ...
    glVertexAttribIPointer(0, 2, GL_SHORT, stride, (GLvoid*)(size_t)(0 * blockAttributeSize));
    glVertexAttribIPointer(1, 2, GL_SHORT, stride, (GLvoid*)(size_t)(2 * blockAttributeSize));
    glVertexAttribIPointer(2, 3, GL_SHORT, stride, (GLvoid*)(size_t)(4 * blockAttributeSize));
    glVertexAttribIPointer(3, 3, GL_SHORT, stride, (GLvoid*)(size_t)(7 * blockAttributeSize));
    glVertexAttribIPointer(4, 4, GL_SHORT, stride, (GLvoid*)(size_t)(10 * blockAttributeSize));
    glVertexAttribIPointer(5, 4, GL_SHORT, stride, (GLvoid*)(size_t)(14 * blockAttributeSize));
    ...
    {
        pushDebugMarker("PCCRenderer Transform feedback layer #1");

        transformFeedbackBegin();
    }
    else
    {
        pushDebugMarker("PCCRenderer Draw layer #1");
    }
    ...
    {
        pushDebugMarker("PCCRenderer Draw layer #2");
    ...
    // Cleanup
    pushDebugMarker("PCCRenderer Cleanup");
...
void PCCRenderer::bindTextures(PresentationFrame& presentationFrame, uint8_t layerIndex)
{
...
void PCCRenderer::unbindTextures(PresentationFrame& presentationFrame, uint8_t layerIndex)
{
    CachedFrame* depth = (layerIndex == 0) ? presentationFrame.depth0 : presentationFrame.depth1;
    CachedFrame* color = (layerIndex == 0) ? presentationFrame.color0 : presentationFrame.color1;
    CachedFrame* occupancy = presentationFrame.occupancy;
...
Texture2D PCCRenderer::createTexture(CachedFrame* frame, bool luma)
{
    Texture2D texture;
...
bool PCCRenderer::dumpFrame(PCCRenderer::PresentationFrame& presentationFrame)
{
    bool result = false;
...
bool PCCRenderer::dumpTexture(std::string filename, Texture2D texture)
{
...
uint8_t* PCCRenderer::readTexture(Texture2D texture)
{
    uint16_t width = texture.width;
    uint16_t height = texture.height;
...
bool PCCRenderer::takeScreenshot(std::string filename, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t bytesPerPixel)
{
    // Alloc
    size_t bytes = width * height * bytesPerPixel;
    uint8_t* buffer = (uint8_t*)malloc(bytes);
...
void PCCRenderer::setupTransformFeedback()
{
#if ENABLE_VERIFICATION_LAYER
...
void PCCRenderer::transformFeedbackBegin()
{
#if ENABLE_VERIFICATION_LAYER
...
void PCCRenderer::transformFeedbackEnd()
{
#if ENABLE_VERIFICATION_LAYER
...
void PCCRenderer::teardownTransformFeedback()
{
#if ENABLE_VERIFICATION_LAYER
...
void PCCRenderer::generateGPUPointCloud(size_t numActiveBlocks, size_t numPoints, std::vector<TFPoint3D> output)
{
#if ENABLE_VERIFICATION_LAYER
...
void PCCRenderer::generateCPUPointCloud(PresentationFrame& presentationFrame, std::vector<TFPoint3D> output)
{
#if ENABLE_VERIFICATION_LAYER
...
bool PCCRenderer::verifyPointCloud(std::vector<TFPoint3D> cpuPoints, std::vector<TFPoint3D> gpuPoints)
{
#if ENABLE_VERIFICATION_LAYER
